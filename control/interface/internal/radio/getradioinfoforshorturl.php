 ...
 *
 * @link http://i.service.t.sina.com.cn/sapps/radio/getradioinfoforshorturl.php
 * @copyright sina.com
...
include_once SERVER_ROOT . 'dagger/libs/model/BaseModelHttp.php';
include_once SERVER_ROOT . 'dagger/libs/extern.php';
class getRadioInfoForShortUrl extends control {
	...
	protected function action() {
		...
		if(!empty($rinfo['title'])&&$rinfo['title']!='微电台'){
			$pre_img = "http://i.service.t.sina.com.cn/radio/radio/getpicforshorturl.php?url=".$this->para['url']."&up=1";
			//增加up参数，为更新动态高清图缓存，没有实际意义的参数
			$huffman_url ="http://service.mix.sina.com.cn/thumbnail/huffman_encode/?format=json&str=".$pre_img;
			$result = json_decode(BaseModelHttp::get($huffman_url), true);
			//$huffman = new MyHuffman(HuffmanDictConfig::$__huffman_dict_v1);
			//$scale62 = $huffman->huffman62Encode($pre_img);
			//$scale62 = "H_".$scale62;	
			$img_prev ="http://service.mix.sina.com.cn/pic/p_46/s_301_122/".$result['result']['data']['scale62'].".jpg";
		}
		$radioInfo['img_prev'] = !empty($img_prev) ? $img_prev : $default_img_url;
		
		$radioInfo['logo_img_url'] = !empty($img_path) ? $img_path : "";
		$data = array();
		if($radioInfo) {	
			$data = array(
				'errno' => 1,
...
new getRadioInfoForShortUrl(RADIO_APP_SOURCE);
?>