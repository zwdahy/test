...
/**
 * Project:     电台管理后台接口
 * File:        set_left_pic.php
 * 
 * 默认轮播大图设置
 * 
 * @link http://i.service.t.sina.com.cn/sapps/radio/set_left_pic.php
 * @copyright sina.com
...
class setLeftPic extends control {
	...
	protected function action() {		
		...
		if ($this->para['type'] == 'set') {
			$picInfoList = $obj->getPicInfo(array('pic_id'=>6));
	...
	if($pic_url!=''&&$link_url!=''){
		...
		if($rs['errorno'] == 1) {	
			...
			$obj->getLeftPic(true);
			$data = array();
			if(empty($resultd['errorno'] ) || $rs['errorno'] == 1) {		
				$data = array(
					'errno' => 1,
					'errmsg' => '成功',
					'result' => $rese
				...
				$data = array(
					'errno' => -9,
					'errmsg' => $_LANG[$resultd['errorno']] != '' ? $_LANG[$resultd['errorno']] : $resultd['errorno']
				);
			}
		}
...
new setLeftPic(RADIO_APP_SOURCE);
?>