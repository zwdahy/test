 ...
 * 添加分类信息
 * 
 * @link http://i.service.t.sina.com.cn/sapps/radio/addpower.php
 * @copyright sina.com
...
class addPower extends control {
	protected function checkPara() {
		...
		$this->para['power'] = request::post('power', 'STR');		//权限级别
		return true;
	}
	protected function action() {
		...
		//通过$uid 查询是否已经在黑名单中了
		$args = array('uid' => $uid);
		$res = $obj->getPower($args);
	
		if($res['result'][0]['uid'] > 0) {
			global $_LANG;
			$data = array(
				'errno' => -9,
				'errmsg' => '用户权限已添加，不需重复添加'
			);
		}else{
			$args = array(
				'uid' => $uid,			
