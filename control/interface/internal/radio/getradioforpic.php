...
class getRadioForPic extends control {
	...
	protected function action() {
		...
		if($result){
			$tmp_radio =array();
			foreach($result['result']['content'] as $k=>$v){
				$tmp_radio[$k]['rid'] = $v['rid'];
				$tmp_radio[$k]['info'] = $v['info'];
			}
		}
		$result['result']['content'] = $tmp_radio;
		$data = array();
		if($result['errorno'] == 1) {
			$data = array(
				'errno' => 1,
...
new getRadioForPic(RADIO_APP_SOURCE);
?>