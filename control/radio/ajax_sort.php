...
class RadioSort extends control {
	protected function checkPara() {
        ...
        $this->para['size'] = intval($_POST['size']);//需要获取的电台数量 获取全部时使用
		//@test
...
//          $radioList = $radioList['result'];
			$this->para['size'] = !empty($this->para['size'])?$this->para['size']:500;
			$radioList = $mRadio->getListenRank(500);
			$data['count'] = count($radioList);
			$radioList = $mRadio->getListenRank($this->para['size']);
...
//			exit;
	...
	//					$v['fm']=$tmp[1];
			...
			$data['count'] = ($data['count']>0)?$data['count']:count($radioList);
		}else{
			$data=array();
		}
		//处理反馈信息
