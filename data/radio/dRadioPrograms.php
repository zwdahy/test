...
class dRadioPrograms extends dRadio{
	...
	public function getRadioProgram($rid,$day,$flag = false){
		$mc_key = sprintf(MC_KEY_RADIO_PROGRAM_NEW,$rid,$day);
		...
		if($final_program == false || $flag == true){
			...
			$this->setCacheData($mc_key,$final_program,MC_TIME_RADIO_PROGRAM_NEW);
		}
	...
	public function getProgramList($rid,$flag = false){
		...
		if($final_program == false || $flag == true){
			...
			if(!empty($programs)){
				...
				$this->setCacheData($mc_key,$final_program,MC_TIME_RADIO_PROGRAM_LIST_NEW);
			}
		}
		return $final_program;
	}
	...
	public function insertRadioProgram($args){
		...
		$sqlArgs = $this->_makeInsert($this->table_name, $args);
		$st = $db->prepare($sqlArgs['sql']);
		if(false == $st->execute($sqlArgs['data'])) {
			$this->writeRadioErrLog(array('执行SQL失败', 'SQL：' . $sqlArgs['sql'], '参数：' . implode('|', $sqlArgs['data']), '错误信息：' . implode('|', $st->errorInfo())), 'RADIO_ERR');
			return $this->returnFormat('RADIO_D_DB_00002');
				...
				if($p_radioinfo['online'] == '1' && $p_radioinfo['program_visible'] == '2'){
					...
					if(!empty($programs)){
						...
						foreach($programs as $day => $p_info){
							$program_info = $p_info['program_info'];
	 ...
	 * @return array
	 */
	public function getProgramForNmaeByRid($rid,$type = 1){
		...
		if($pagecount > 0){
			...
			for($page=1;$page<=$pagecount;$page++){
				...
				foreach($result['rs'] as $rs_value){
					$programinfo = $this->getProgramForNmaeByRid($rs_value['vradio_id'],0);
	...
	*/
	public function getNameCard($uid,$pname = ''){
		...
		if($result == false){
			...
			foreach($user_info as $value){
				$result = array(
						...
						'program_name' => !empty($user_program_info) ? $user_program_info : array(),
						'relation' => false
				);
			}
		...
		$user_program_info = $this->getProgramByUid($uid);
		$new_program = $user_program_info['program_name'];
		$old_program = $result['program_name'];
		if($new_program){
			foreach($new_program as $v){
				if(!in_array($v,$old_program)){
					$is_new = true;
					break;
				}
			}
		...
		if(!$is_new){
			if($old_program){
				foreach($old_program as $v){
					if(!in_array($v,$new_program)){
						$is_new = true;
						break;
					}
				}
		...
		$cuid = $cuserInfo['uid'];
		if($cuid > 0){
			$relation = $this->checkattrelation($cuid,$uid);
			$result['relation'] = $relation['one2many'][$uid];
		}
		return $result;
	}
	...
	public function updateHotProgram(){
		...
		if($pagecount > 0){
			...
			for($page=1;$page<=$maxpage;$page++){
				$result = $this->getHotProgram($begintime,$endtime,2,50,$page);
				foreach($result['rs'] as $rs_value){
					$programinfo = $this->getProgramForNmaeByRid($rs_value['vradio_id'],0);
