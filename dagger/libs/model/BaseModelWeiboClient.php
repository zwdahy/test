<?php
/**
 * PHP SDK V2 for open.weibo.com (using OAuth2\Cookie\Basic)
 * 微博数据操作类SDK V2
 * 支持Cookie认证、类Basic认证及Oauth2认证
 * 
 * 使用说明:
 * 1、Cookie认证，应用在当前用户的操作，只需要提供appkey
 *  //需要当前用户已登录并且开通了微博账号，通常用于当前用户发微博、转发、加关注等操作。
 *	$weibo = new WeiboClient($appkey)
 *  $data = $weibo->user_timeline_by_id($uid);
 *
 * 2、类Basic认证，即已知指定账号的用户名和密码，只需要提供appkey。
 *  //程序模拟指定账号登录，通常用在当前用户未开通微博、未登录或其他无法取到用户身份的情况下，获取需要的信息或列表。
 *	$weibo = new WeiboClient($appkey)
 *  $weibo->set_user($username, $password);
 *  $data = $weibo->user_timeline_by_id($uid);
 *	$weibo->unset_user();
 *
 * 3、OAuth2认证，需要提供完整的appkey,app_secret,oauth_access_token。目前未启用refresh_token
 *  //需要弹出窗口，用户确认应用授权才能取到oauth_access_token，一般用于站外微博应用。
 *  //OAuth2认证相关文档：http://open.weibo.com/wiki/Oauth2 
 *  $weibo =  new WeiboClient($appkey,$app_secret,$oauth_access_token)
 *  $data = $weibo->user_timeline_by_id($uid);
 * 
 * 
 * @author Easy Chen, Elmer Zhang, Lazypeople, Roryliu
 * @version 1.1
 * @update 2011/12/06 11:15:00
 */
class BaseModelWeiboClient{
	/**
	 * 构造函数
	 * 
	 * @access public
	 ...
	 * @return void
	 */
	function __construct( $akey, $skey = NULL, $access_token = NULL, $refresh_token = NULL)
	{
		$this->oauth = new libWeiboOAuthV2($akey, $skey, $access_token, $refresh_token);
	}
	 ...
	 *
	 * @access public
	 * @param string $username 用户账号的用户名，一般是邮箱形式。
	 * @param string $password 用户账号的密码。
	 * @return void
	 */
	function set_user($username, $password){
            $mc = new BaseModelMemcache;
            $cookie = $mc->get('sso_login_cookie_' . $username);
            if (empty($cookie) || strpos($cookie, "SUE=deleted") !== false) {
                srand((double)microtime()*1000000);
			    $ip = "11." . rand(1,254) . "." . rand(1,254) . "." . rand(1,254);
			    $ip = $_SERVER["REMOTE_ADDR"] ? $_SERVER["REMOTE_ADDR"] : $ip;
                $sso_pin = SINA_SSO_PIN;
                $ssologin_url = 'http://ilogin.sina.com.cn/api/chksso.php?source=topit_t_weibo_sdk_v2&entry='.SINA_SSO_ENTRY.'&user='.$username.'&pw='.$password.'&ip='.$ip.'&m='.md5($username.$password.$ip.$sso_pin);
                $stream_context = stream_context_create(array('http' => array('timeout' => 5)));
                $ret = file_get_contents($ssologin_url, false, $stream_context);
                $ret = substr($ret,strpos($ret,"result"));
                BaseModelCommon::debug($ret, '==sso_login_result==');
                parse_str($ret, $result);
                BaseModelCommon::debug($result, 'sso_login_result');
                $cookie = '';
                if($result['result'] == 'succ'){
                    preg_match_all('/Set-Cookie: (.*?=.*?;)./m', $result['cookies'], $regs);
                    foreach ($regs[1] as $v) {
                        $cookie .= $v;
                    }
                    $mc->set("sso_login_cookie_" . $username, $cookie, 3600);
                }
            }
            $this->oauth->setCookie($cookie);
	}
	...
	* @return mixed
	*/
	function account_weihao($weihao){
		$params = array();
		$params['number'] = $weihao;
		return $this->oauth->get( 'account/weihao', $params );
	}
	 ...
	 * @return array
	 */
	function timeline_batch_by_id( $uids, $page = 1, $count = 50, $feature = 0, $base_app = 0, $since_id='', $max_id='')
	{
		$params = array();
	...
	function exists_batch_internal($target_ids, $source_id = NULL)
	{
		$params = array();
		$params['uids'] = $target_ids;
		if ( $source_id != NULL ) {
		$this->id_format($source_id);
		$params['uid'] = $source_id;
		}

	return $this->oauth->get( 'friendships/exists_batch_internal', $params );
	}
	 ...
	 * @return array
	 */
	function search_suggestions_users( $q,  $count = 10 )
	{
		$params = array();
		$params['q'] = $q;
		$params['count'] = $count;
     ...
     * @return array 
     */
    function register_v1($nick, $gender, $password, $email, $ip, $province = NULL, $city = NULL)
    {
        $params = array();
        $params['nick'] = $nick;
     ...
     * @return array 
     */
    function activate_v1($uid, $nickname, $gender, $email, $ip, $province, $city = NULL)
    {
        $params = array();
        $params['uid'] = $uid;
        $params['nickname'] = $nickname;
     ...
     * @return array 
     */
    function search_users_v1($q, $page = 1, $count = 10, $extra = array() )
    {
        $params = array();
        $params['q'] = $q;
     ...
     * @return array 
     */
    function search_status_v1($q, $page = 1, $count = 10, $extra = array() )
    {
        $params = array();
        $params['q'] = $q;
        if ($page) $params['page'] = $page;
     ...
     * @return array 
     */
    function search_v1($q, $page = 1, $count = 10, $extra = array() )
    {
        $params = array();
        $params['q'] = $q;
        if ($page) $params['page'] = $page;
     ...
     * 
     * @access public
     * @param string $short_url 需要获取的短链接，需要URLencoded，最多不超过20个 。
     * @return array 
     */
    function short_url_batch_info_v1( $short_url )
    {
        $params = array();
        $params['url_short'] = $short_url;
 
        return $this->get('http://api.t.sina.com.cn/short_url/batch_info.json', $params);
    }
     ...
     * @return array 
     */
    function short_url_batch_info_v2( $short_url )
    {
        $params = array();
        $params['url_short'] = $short_url;
        return $this->get('http://i2.api.weibo.com/2/short_url/info.json', $params, $multidimensionalWeibo=true);
    }
     ...
     * 
     * @access public
     * @param string $short_url 需要还原的短链接，需要URLencoded，最多不超过20个 。
     * @return array 
     */
    function short_url_expand_v1( $short_url )
    {
        $params = array();
        $params['url_short'] = $short_url;
 
        return $this->get('http://api.t.sina.com.cn/short_url/expand.json', $params);
    }
    ...
    /**
	 * 批量获取用户发布的微博信息列表（代理接口）
     * 
     * @access public
     * @param string $uids 需要获取微博的用户ID，以逗号隔开。例如：1220291284,1191965271
	 * @param int $count 每个用户获取微博列表的条数，默认5 
	 * @param int $feature 微博类型，0全部，1原创，2图片，3视频，默认0。
     * @return array 
     */
    function user_timeline_batch_proxy( $uids, $count = 5, $feature = 0)
    {
        $params = array();
        $params['uid'] = $uids;
        if ($count) $params['count'] = $count;
        ...
        return $this->get('http://api.sina.com.cn/weibo/wb/user_timeline.json', $params);
    }

    /**
     * 批量获取指定的一批pid的图片信息
     *
     * 对应API：{@link http://i2.api.weibo.com/images/show_batch.json}
     *
     * @param string $screen_name  需要查询的用户昵称，用半è逗号分隔，一次最多20个
     * @return array
     */
    function imagesinfo_batch_by_id( $pids )
    {
        $params=array();
        if (is_array($pids) && !empty($pids)) {
            ...
            $params['pids'] = join(',', $pids);
        } else {
            $params['pids'] = $pids;
        }
        return $this->oauth->post('images/show_batch', $params);
    }

    function user_timeline_by_id_show_ids($uid, $page = 1 , $count = 50 , $since_id = 0, $max_id = 0, $feature = 0, $trim_user = 0, $base_app = 0){
        $params         = array();
        $params['uid'] = $uid;
        if ($since_id) {
            $this->id_format($since_id);
        ...
        $base_appparams['base_app'] = intval($base_app);
        $params['feature'] = intval($featurefeature);
        $params['count'] = intval($count);
        $params['page'] = intval($page);
        $params['trim_user'] = intval($trim_user);
       ...
       /**
        * http://open.weibo.com/wiki/2/users/count
        * 批量获取指定微博的转发数评论数 
        * @param : ids
        */
        function user_count( $uids )
        {       
            $params = array();
            $params['uids'] = $uids;
         ...
         * 长微博
         */
        function upload_url_text_image($status,$url)
        {
            $params = array();
            $pidsarams['status'] = $status;
            $params['url'] = $url;
	 ...
	 * @ignore
	 */
    private function get($url, $params = array(), $multidimensionalWeibo=false) {
        if ( !isset($params['format']) || $params['format'] != 'json' ) {
            $params['format'] = 'json';
        }
        if ($params) {
            if (!$multidimensionalWeibo) {
                ...
                $url .= '?' . implode('&', $paramArr);
            }
        }
        $response = $this->oauth->http($url, 'GET');
        $response = json_decode($response, true);
        BaseModelCommon::debug($response,'api_response');
        return $response;
    }
...
class libWeiboOAuthV2 {
	 ...
	 * @ignore
	 */
	public $useragent = 'Weibo OAuth2 v0.1';
	 ...
	 */
	function __construct($client_id, $client_secret = NULL, $access_token = NULL, $refresh_token = NULL) {
	...
	function http($url, $method, $postfields = NULL, $headers = array()) {
		...
		BaseModelCommon::debug($postfields,'post_data');
		BaseModelCommon::debug($headers,'header_data');
		BaseModelCommon::debug($url,'api_url');	
		BaseModelCommon::debug(curl_getinfo($ci), 'api_curl_info');
