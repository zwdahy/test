...
class BaseModelXML {
    ...
    protected function array2Xml($arr, $lvl = 0, $prev_lvl = 0) {
        ...
        foreach ($arr as $key => $value) {
            ...
            if (!is_array($value)) {
                //first node should start at a new line
                if ($prev_lvl != $lvl) {
                    $xmlStr .= $lineBreakChar;
                    $prev_lvl = $lvl;
                }

                // the node is a leaf,  so xml tag should be <key><value></key>
                $xmlTag = sprintf("<%s>%s</%s>", $key, $this->strValueToXMLSafe($value), $key);
                ...
                if ($associative) {
                    ...
                    foreach ($value as $valueTag) {
                        if (is_array($valueTag)) {
                            ...
                            $xmlStr .= $this->indent($lvl) . '<' . $key . '>';
                            $xmlStr .= (string) $valueTag;
                            $xmlStr .= '</' . $key . '>' . $lineBreakChar;
                        }
                    }
                }
            }
