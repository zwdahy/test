...
/*
    $html = '<a href="http://radio.weibo.com/china/fm887?source=notice"target="_blank">@HITFM 正在播出#Big Drive Home#，欢迎收听！>></a>';
    $htmlDom = new HtmlParserModel($html);
    //print_r($htmlDom->child[1]->child[0]);
    $str = json_encode($htmlDom->child[1]->child[0]);
...
for($page=0; $page<=40; $page++){
    $start = $page*25; 
    $html = file_get_contents('http://www.douban.com/group/beijingzufang/discussion?start='.$start);
    $htmlDom = new HtmlParserModel($html);
    $arr = $htmlDom->find('td.title a');
    
    //$arr_author = $htmlDom->find('td.wrap a');
    foreach ($arr as $p){
        if( false == match_my_desire($p->title) ) continue;
        $tmp[$p->title] =  $p->href;
    }
}
...
print_r($tmp);
*/
function match_my_desire($title){
   $include_words = array('中关村', '苏州街', '五道口', '魏公村', '人民大学', '民大', '民族大学', '北航', '稻香');
   $exclude_words = array('女生');
